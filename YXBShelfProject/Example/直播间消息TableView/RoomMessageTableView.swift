...
//    @objc lazy var topview: UIView = {
//        let topview = UIView(frame: CGRectMake(0, 0, ScreenConst.width, 15))
//        topview.applyGradientMask(forFadeLength: 15)
//        return topview
//    }()
...
//        self.makeToast("点击了头像")
    ...
    // MARK: 点击了昵称，只有进入房间的那两个消息类型触发了
    func tableView(cell: LQMessageCell, didTapNickNameModel: LQMessageModel) {
        self.delegate?.tableViewdidTapNickName?(model: didTapNickNameModel)
    }
}

private extension RoomMessageTableView {
    // 是为了在应对息屏期间收到进房消息绘制不出等级图标。
