...
//  CUYuYinFang
//
//  Created by yangxiaobin on 2023/10/19.
...
class MSDressViewModel {
    ...
    func fetchData(type: MSDressType, closure: @escaping () -> ()) {
        ...
        network.sendRequest(.findZhuangbanProdList(type: type.rawValue), hotPlugins: [.progress]) { [weak self] obj in
    ...
    // 我的背包
    func fetchMyBageData(type: MSDressType, closure: @escaping () -> ()) {
        let network = NetworkManager<MyAPI>()
        network.sendRequest(.findMyZbList(type: type.rawValue)) { [weak self] obj in
            guard let self = self else { return }
            let array: [MSDressModel] = jsonToArray(jsonData: obj)
            self.modelArray.accept(array)
            self.seletArray.accept(Array(repeating: false, count: array.count))
