...
//            } else {
            ...
            // VAP的播放控件有点问题，源码里判断view是否被添加且被展示了，被展示了才会播放出来。所以做了一个延迟操作。
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                self.vapPlayerView.startVap(urlString: url, userInfo: ["nickname": UserConst.nickName, "headImg": UserConst.headImg])
            }
