...
protocol MSDressBuyDeleagte: NSObjectProtocol {
    func didBuySuccess()
}

class MSDressBuyVC: UIViewController {
    
    weak var delegate: MSDressBuyDeleagte?
    ...
    let moneyButton: YXBButton = {
        let button = MyUIFactory.commonImageTextButton(title: UserConst.miZuan, titleColor: .titleColor, titleFont: .titleFont_14, image: UIImage(named: "CUYuYinFang_shangcheng_图层 27"), space: 2)
        return button
    }()
        ...
        if let zbId = self.model.id,
           ...
           let subId = self.viewModel.modelArray.value[index].id  {
            viewModel.requestBuyDress(zbId: zbId, subId: subId) { [weak self] in
                self?.delegate?.didBuySuccess()
                self?.dismiss(animated: true)
            }
        } else {
            self.view.makeToast("请选择价格档位", position: .center)
        }
    }
